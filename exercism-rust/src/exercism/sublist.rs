...
pub fn sublist(first: &[i32], second: &[i32]) -> Comparison {
    ...
    match (first.len(), second.len()) {
        (a, b) if a == b => {
            if first == second { Equal } else { Unequal }
        }
        (0, _) => Sublist,
        (_, 0) => Superlist,
        (a, b) if a < b => {
            if contains(first, second) { Sublist } else { Unequal }
        }
        _ => {
            if contains(second, first) { Superlist } else { Unequal }
        }
    }
}

fn contains(smallest_list: &[i32], largest_list : &[i32]) -> bool {
    largest_list
        .windows(smallest_list.len())
        .any(|window| window == smallest_list)
}
