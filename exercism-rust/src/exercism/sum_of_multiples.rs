...
pub fn sum_of_multiples(limit: u32, factors: &[u32]) -> u32 {
    // Deduplicate the `factors` input.
    // It is not asked, neither tested, but seems pertinent as there could be items with same factors
    // in the input. So we do not want to compute the `multiple_factors` twice or more.
    let uniq_factors: HashSet<u32> = factors
        .iter()
        .filter(|&&x| x != 0)// Manage case 0 in order to avoid a zero-division
        .copied()
        .collect();
    let uniq_multiples: HashSet<u32> = uniq_factors
        .iter()
        .flat_map(|&factor| (factor..limit).step_by(factor as usize)) // Compute multiples
        .collect();
