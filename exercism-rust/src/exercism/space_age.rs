...
pub trait Planet {
    ...
    fn years_during(d: &Duration) -> f64 {
        d.seconds as f64 / EARTH_YEAR_IN_SECONDS as f64 / Self::orbital_period_in_earth_years()
    }
}

// Use a `macro_rules!` to generate the struct `Planet` and the trait implementation.
macro_rules! planet {
    ($n:ident, $p:expr) => {
        pub struct $n;
        impl Planet for $n {
