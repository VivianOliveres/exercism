...
pub fn anagrams_for<'a>(word: &str, possible_anagrams: &[&'a str]) -> HashSet<&'a str> {
    ...
    let word_letter_counts: HashMap<char, u32> = to_map(&word);
    possible_anagrams
        .iter()
        .filter_map(|&candidate| { // filter && map both needs `&candidate.to_lowercase()` so let use `filter_map`
            let candidate_lower_case = &candidate.to_lowercase();
            if &word == candidate_lower_case {
                None
            } else {
                Some((candidate, to_map(candidate_lower_case)))
            }
        })
        .filter(|tuple| tuple.1 == word_letter_counts)
        .map(|tuple| tuple.0)
        .collect::<HashSet<&str>>()
}

fn to_map(word: &str) -> HashMap<char, u32> {
