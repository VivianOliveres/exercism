...
// An alternative solution is to only keeps minutes.
// Then compute hours when `to_string` is called.

impl Clock {
    pub fn new(hours: i32, minutes: i32) -> Self {
        let (hours, minutes) = Self::compute(hours, minutes);
        Clock { hours, minutes }
    }

    fn compute(hours: i32, minutes: i32) -> (i32, i32) {
        let total_minutes = hours * HOUR + minutes;
        let abs = total_minutes.rem_euclid(DAY);
        (abs / 60, abs % 60)
    }

    pub fn add_minutes(mut self, minutes: i32) -> Self {
        let (added_hours, added_minutes) = Self::compute(self.hours, self.minutes + minutes);
